 ...
 *
 * Khusus mode 'betting', seberapa banyak isi soal yang boleh dilihat siswa
 * saat memasang taruhan diatur host lewat room.options.betInfo:
 * 'full' (soal + kode), 'clue' (pertanyaan + petunjuk dari bank soal), atau
 * 'blind' (cuma jenis kartu & level).
 */
...
// Seberapa banyak isi soal yang ditampilkan selama fase taruhan:
// - 'full'  : pertanyaan + blok kode (opsi jawaban tetap ditahan). Default,
//             sama seperti perilaku pertama fitur taruhan.
// - 'clue'  : pertanyaan + petunjuk singkat dari field `clue` bank soal;
//             blok kodenya ditahan sampai fase menjawab.
// - 'blind' : cuma jenis kartu & levelnya - taruhan jadi taksiran terhadap
//             topik, bukan terhadap soal spesifik.
const BET_INFO_LEVELS = new Set(['full', 'clue', 'blind'])

function normalizeMode(mode) {
	return MODES.has(mode) ? mode : 'classic'
}

function normalizeBetInfo(info) {
	return BET_INFO_LEVELS.has(info) ? info : 'full'
}
...
function createGame(banks, defaultBank, deps) {
	...
	function createRoom(hostConn, opts = {}) {
		...
		const room = {
			...
			options: {
				...
				// Cuma dipakai kalau mode = 'betting'.
				betInfo: normalizeBetInfo(opts.betInfo),
	 ...
	 *                Berapa banyak sisa isi soal yang ikut dikirim tergantung
	 *                room.options.betInfo (lihat BET_INFO_LEVELS).
	...
	function cardPayload(room, q, timeLimit, { stage = 'question' } = {}) {
		...
		if (stage === 'bet') {
			const betInfo = normalizeBetInfo(room.options.betInfo)
			payload.betInfo = betInfo
			...
			// 'blind' cukup dengan jenis kartu & level yang sudah ada di atas.
			if (betInfo !== 'blind') payload.prompt = q.prompt
			if (betInfo === 'full') payload.code = q.code
			// Petunjuk SENGAJA tidak dibuat otomatis dari isi kode: ringkasan
			// mesin gampang membocorkan jawaban (mis. menyebut operator yang jadi
			// kunci di soal Cari Bug). Jadi nilainya diambil apa adanya dari field
			// `clue` di bank soal - soal yang belum punya clue tetap jalan, siswa
			// cuma diberi tahu kartu itu tanpa petunjuk.
			if (betInfo === 'clue') payload.clue = q.clue || null
		} else {
	...
	function handle(conn, msg) {
		...
		switch (msg.t) {
			...
			case 'host:options': {
				...
				room.options.betInfo = normalizeBetInfo(room.options.betInfo)
